...
sub gameLoop()
    ...
    if m.gameState = "running" then
        ...
        ' else if m.isJumping= "true"
        '     m.playerImage.uri = "pkg:/images/dinorun/f" + m.runFrameIndex.toStr() + ".jpg"
        ' end if

    ' --- RUNNING ANIMATION ---
if m.isJumping = true then
    ' Use a specific jump frame
    m.playerImage.uri = "pkg:/images/dinorun/f5.jpg"
else
    ' Running logic
    if m.animFrameCount MOD 5 = 0 then
        m.runFrameIndex = m.runFrameIndex + 1
    ...
    ' 3. Apply Shake Offset to Y-Coordinate
    ' This keeps the X moving, but adds the shake to the Y-translation
    baseY = m.obstacle.translation[1]
    
    ' Logic to find the original Y based on the spawner logic (300, 400, or 435)
    ' This ensures it returns to its intended height after the shake
    originalY = 400 ' Default fallback
    if m.obs4.visible then originalY = 300
    if m.obs2.visible or m.obs3.visible then originalY = 435
    
    ' Apply shake to the Y position
    m.obstacle.translation = [oX, originalY + m.shakeOffset]
end if
...
if playerLeft < obsRight and playerRight > obsLeft and playerTop < obsBottom and playerBottom > obsTop then
    ...
    ' Check if Dino is in the air
    if m.isJumping = true then
        m.gameState = "falling" ' New state: fall to ground before crashing
    else
        m.gameState = "crashing" ' If on ground, crash immediately
    end if
