...
sub onTimerFire()
    ...
    ' --- UNSTOPPABLE GRAVITY LOOP ---
    ' Runs every single frame independently of the shooting states
    for i = m.fallingBubbles.count() - 1 to 0 step -1
        node = m.fallingBubbles[i]
        oldPos = node.translation
        newY = oldPos[1] + 15 ' Gravity speed
    ...
    ' --- -100 TRICK FIX: Update Roof Collision ---
    if newY <= (m.baseY + m.scrollOffset - 100) then
        hasCollided = true
    else
        ' 2. Bubble Collision
